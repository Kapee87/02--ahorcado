...
function alternarVisibilidadOff() {
    ...
    $facil.classList.remove('hidden');
    $medio.classList.remove('hidden');
    $dificil.classList.remove('hidden');
    ...
    if (esValido(e.which) && !repetida.includes(chr)) {
        ...
        if (!repetida.includes(chr)) {
            ...
            if (pifiar) {
                ...
                intentos++;
                repetida.push(chr);
                return !verificarGanador(repetidAcertada, pifiar) ? verificarPerdedor(pifiar) : "";
            } else {
